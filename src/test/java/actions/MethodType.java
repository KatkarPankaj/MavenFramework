...
public class MethodType {
	...
	public void methodExecutor(String methodType, String objectLocators,
			String actionType, String data) throws Exception {
...
extendReport ss = new extendReport();
		...
		case "CSS":
			...
			break;
		case "LINKTEXT":
			findElementLinkText(objectLocators);
			mModel.setElement(listOfElements);
			findMethod(methodType, objectLocators, actionType, data, mModel);
			break;
	//	case "URL":
	//	navigateToURL(mModel);
	//	findMethod(methodType, objectLocators, actionType, data, mModel);
	//	break;
		default:
	...
	public void findMethod(String methodType, String objectLocators,
		...
		try {
			...
			String cause = e.getTargetException().toString();
		
		
			Throwable cause1 = e.fillInStackTrace();
			String n = cause1.getLocalizedMessage();
			StackTraceElement[] eee = e.getStackTrace();
Assert.fail("Test case is failed because of-----:" +cause);
			//Assert.fail("Test case is failed cause of---", cause);
			System.out.println("exception occured in finding methods, method name is incorrect"
							+ e);
		}
	...
	public void findElementLinkText(String objectLocators) {

		WebDriverWait wait = new WebDriverWait(WebDriverClass.getDriver(), 30);
		List<WebElement> list1 = wait.until(ExpectedConditions
				.presenceOfAllElementsLocatedBy(By.linkText(objectLocators)));
	...
	public void click(MethodParameters model) {
		...
		System.out.println("click method started"
				+ model.getObjectLocators());
		System.out.println("click method completed");
	}
	...
	public void submit(MethodParameters model) {
		System.out.println("submit method started"
				+ model.getObjectLocators());
		model.getElement().get(0).submit();
		System.out.println("submit method end");
	}
	...
	public void readTextFieldValue(MethodParameters model) {
		System.out.println("inside readTextFieldValue()"
				+ model.getObjectLocators());
		model.getElement().get(0).getAttribute("value");
		System.out.println("end of readTextFieldValue");
	}
	...
	public void alertAccept(MethodParameters model) {
		...
		System.out.println("inside alertAccept()");
		...
		alert.accept();
		System.out.println("completed alertAccept()");
	}
	...
	public void alertDismiss(MethodParameters model) {
		...
		System.out.println("inside alertDismiss()");
		wait1(2000);
	 ...
	 */
	public void verifyURL(MethodParameters model) {
		System.out.println("inside verifyTitleOfPage()" + "title=="
				+ WebDriverClass.getInstance().getCurrentUrl() + "data from excel="
				+ model.getData());

		wait1(2000);
		st = new SoftAssert();
		
		String actual = WebDriverClass.getInstance().getCurrentUrl().toString();
		String expected = model.getData().toString();
		Assert.assertTrue(actual.contains(expected), "Verifycation failed as URL is incorrect... "+ actual);
	...
	public void verifyTitleOfPage(MethodParameters model) {
		...
		String expected = model.getData().toString();
		Assert.assertEquals(actual, expected, "Xyz");
		System.out.println("assert verification successful verifyTitleOfPage()");
	...
	public void selectDropDownByIndex(MethodParameters model) {
		System.out.println("inside selectDropDownByIndex");
		Select sel = new Select(model.getElement().get(0));
	...
	public void selectDropDownByValue(MethodParameters model) {
		System.out.println("inside selectDropDownByValue");
		Select sel = new Select(model.getElement().get(0));
	...
	public void switchToFrame(MethodParameters model) {
		...
		WebDriverClass.getInstance().switchTo()	.frame(model.getElement().get(0));
	...
	public void selectFromListDropDown(MethodParameters model) {
		System.out.println("inside selectFromListDropDown");
		wait1(2000);
	...
	public void readText(MethodParameters model) {
		System.out.println("getText() method called  and value of getText==*************"
						+ model.getElement().get(0).getText());
		model.getElement().get(0).getText();
		System.out.println("readText completed");
	}
	/**
	...
	public void scrollElementIntoView(MethodParameters model) {
		...
		System.out.println("scrollElementIntoView started");
	...
	public void switchToWindow(MethodParameters model) {
		System.out.println("inside switchToFrame");
		WebDriverClass.getInstance().switchTo().window(model.getData());
	...
	public void OpenMuhimbiApp(MethodParameters model) throws InterruptedException {
		Thread.sleep(4000);
	
		WebDriverClass.getDriver().findElement(By.id("O365_MainLink_Settings")).click();
	
		Thread.sleep(3000);
		WebDriverClass.getDriver().findElement(By.linkText("Site contents")).click();

		Thread.sleep(3000);
		WebDriverClass.getDriver().findElement(By.linkText("Muhimbi PDF Converter for SharePoint Online")).click();
